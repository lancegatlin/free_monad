...
object Try5 {
  ...
  trait Api {
    ...
    def run[A](ast: AST[A]) : A = {
      ast match {
        case Value(a) => a
        case Applicative(ast1,ast2) => (run(ast1),run(ast2))
  ...
  object ~ { def unapply[A,B](tuple:(A,B)) : Option[(A,B)] = Some(tuple) }
  ...
  trait PersonService {
    ...
    def updateAddress(id: Int, newAddress: String) : AST[Option[Person]] = {
      for {
        // Not sure why but (optPerson,_) gives compiler error
        tuple <- personDao.findById(id) |+| personDao.findById(0)
        (optPerson,_) = tuple
